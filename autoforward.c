...
static void remove_autoforward_cb(void);

void plugin_load(void)
{
	debug_print("initializing autoforward_tool plug-in\n");

	syl_plugin_add_menuitem("/Tools", NULL, NULL, NULL);
	syl_plugin_add_menuitem("/Tools", _("Remove autoforwards"), remove_autoforward_cb, NULL);
...
static gboolean remove_autoforward_rec(MimeInfo *partinfo, FILE *fp, FILE *outfp)
{
	gchar buf[BUFFSIZE];
	gchar *boundary = NULL;
	gint boundary_len = 0;
	...
	while (partinfo) {
		...
		if (partinfo->sub) {
			...
			err = !remove_autoforward_rec(partinfo->sub, fp, outfp);
			debug_print("leave rfc822 -----------------------------\n");
			if (err)
				break;
		} else if (partinfo->children) {
			debug_print("enter child -----------------------------\n");
			err = !remove_autoforward_rec
...
static gboolean remove_autoforward(MsgInfo *msginfo)
{
	MimeInfo *mimeinfo;
	FILE *fp, *outfp;
	...
	if (!has_attach_part(mimeinfo)) {
		debug_print("remove_autoforward: this message doesn't have autoforwards\n");
	...
	err = !remove_autoforward_rec(mimeinfo, fp, outfp);
...
static void remove_autoforward_cb(void)
{
	...
	debug_print("remove_autoforward_cb\n");
	...
	val = syl_plugin_alertpanel(_("Remove autoforwards"),
	...
	for (cur = mlist; cur != NULL; cur = cur->next) {
		...
		debug_print("Removing autoforwards of message: %u: %s\n", msginfo->msgnum, msginfo->subject ? msginfo->subject : "(No Subject)");

		if (!remove_autoforward(msginfo)) {
			g_warning("Cannot remove autoforwards from %u: %s\n", msginfo->msgnum, msginfo->subject ? msginfo->subject : "(No Subject)");
			break;
		}
